...
useEffect(() => {
  ...
  if (checkoutStatus === 'success') {
    console.log('✅ Checkout success detected, waiting for tier update...');
    ...
    const checkTierUpdate = async (attempts = 0) => {
      ...
      if (attempts >= maxAttempts) {
        console.log('⚠️ Max attempts reached, forcing reload...');
      ...
      try {
        ...
        const response = await fetch('/api/auth/session?update=1');
        const session = await response.json();
        
        console.log(`🔄 Attempt ${attempts + 1}: Checking tier...`, session?.user?.tier);
        
        // Si el tier cambió de 'free', la actualización funcionó
        if (session?.user?.tier && session.user.tier !== 'free') {
          console.log('✅ Tier updated successfully! Reloading...');
          window.location.reload();
        } else {
    ...
    // Esperar 2 segundos antes de empezar a verificar
    // (dar tiempo al webhook de procesar)
    setTimeout(() => checkTierUpdate(), 2000);
  }
}, []);
